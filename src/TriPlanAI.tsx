...
import { Download, Settings2, RefreshCw, Calendar as CalendarIcon, Bike, Dumbbell, StretchHorizontal, Zap, CheckCircle2, Info, X, WifiOff } from "lucide-react";
...
function WeekView({ w, toggleMissed }: { w: WeekPlan; toggleMissed: (id: string) => void }) {
  ...
  return (
    <div className="rounded-3xl border shadow-md bg-white p-6">
      ...
      {/* Responsive days: horizontal scroll on small, grid on large */}
      ...
      <div className="hidden xl:grid xl:grid-cols-7 gap-6">
        {dayOrderMonStart.map((idx) => (
          <div key={idx} className="rounded-2xl border bg-gradient-to-b from-gray-50 to-white p-5 shadow-sm">
            <div className="font-medium text-sm mb-3">{dayNames[idx]}</div>
            <div className="space-y-6">
              {(grouped[idx] || []).map(s => (
                <SessionCard key={s.id} s={s} toggleMissed={toggleMissed} />
        ...
        {settingsCollapsed ? (
          <div className="grid lg:grid-cols-3 gap-6 mb-8 transition-all duration-300">
            ...
            <div className="rounded-3xl border bg-white p-6 shadow-sm lg:col-span-3 transition-all duration-300">
              ...
              <WeekView w={plan[activeWeek]} toggleMissed={toggleMissed} />
            </div>
          </div>
        ) : (
          <div className="grid lg:grid-cols-3 gap-6 mb-8 transition-all duration-300">
